    ...
    return function makeQuestion({type, question, answers} = {}) {
        ...
        if (!type) throw new Error('A question must have a type.');

        if (typeof type !== 'string') throw new Error('The type must be a string.');

        if (!question) throw new Error('The question field is required.');

        if (typeof question !== 'string') throw new Error('A question must be a string required.');

        if (!answers) throw new Error('A question must have answers.');
