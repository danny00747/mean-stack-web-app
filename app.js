...
var _passport2 = _interopRequireDefault(require("./dist/security/passport"));

var _routes = _interopRequireDefault(require("./dist/routes"));
...
require('./dist/config/database'); // Initialize the app
...
if ((process && process.env && process.env.NODE_ENV || "production") === 'production') {
  app.use(_express.default.static(_path.default.join(__dirname, 'server-side', 'public', 'dist')));
...
app.get('/', (req, res) => {
  return res.sendFile(_path.default.join(__dirname + '/server-side', 'public', 'dist', 'index.html'));
...
const logs = require('./dist/routes/logs');
