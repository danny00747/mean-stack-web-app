...
const users_get_all = async (req, res) => {
  await logs.saveLog('info', req, 'Incoming');
  User.find().select("-__v").exec().then(users => {
    if (users.length === 0) {
...
const update_user = async (req, res) => {
  ...
  if (userId === (process && process.env && process.env.ADMIN_ID || "5e7fd9abf35b123cbc246898") && req.user.role !== 'admin') {
...
const update_user_score = async (req, res) => {
  ...
  const userScore = {
    ...
    "level": getLevel(req.user, req.body.score)
  };
  User.updateOne({
    _id: userId
  }, {
...
const get_user_by_id = async (req, res) => {
  ...
  User.findById(userId).select("_id email role username level password").exec().then(doc => {
    if (doc) {
      logs.updateLog('info', req, 'Outgoing', 200);
      res.status(200).json({
        user: {
