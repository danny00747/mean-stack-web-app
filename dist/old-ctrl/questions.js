...
const question_get_all = async (req, res) => {
  ...
  let response = [];
  Question.find().select("question answers _id type").exec().then(docs => {
    if (docs.length === 0) {
...
const question_get_one = async (req, res) => {
  ...
  Question.findById(questionId).select("question answers _id").exec().then(doc => {
    if (doc) {
      logs.updateLog('info', req, 'Outgoing', 200);
...
const question_delete_one = async (req, res) => {
  ...
  Question.findByIdAndRemove(questionId).exec().then(doc => {
    if (doc === null) {
      logs.updateLog('info', req, 'Outgoing', 404);
      return res.status(404).json({
        message: "No valid entry found for provided ID"
    ...
    logs.updateLog('error', req, 'Outgoing', 500);
    res.status(500).json({
      message: "An error occured while trying to delete this question"
    });
  });
};
