...
const getAuthor = (req, res, callback) => {
  ...
  User.findById(userId).select("_id email password").exec().then(doc => {
    if (doc) {
      callback(req, res);
    } else {
      res.status(404).json({
...
const reviewsUpdateOne = (req, res) => {
  ...
  if (!userEmail || !reviewId) {
    return res.status(404).json({
      message: 'The question id and review id are both required'
    });
  }

  User.findOne({
    email: userEmail
  }).select('reviews').exec().then(user => {
    ...
    if (user.reviews && user.reviews.length > 0) {
      ...
      if (!user.reviews.id(reviewId)) {
        return res.status(404).json({
          message: 'No was review found with provided ID'
        });
      } else {
...
const reviewsDeleteOne = (req, res) => {
  ...
  if (!userEmail || !reviewId) {
    return res.status(404).json({
      message: 'User email and review id are both required'
    });
  }
