...
describe('E2E TESTS FOR LOGIN PAGE', async () => {
  ...
  describe('/POST Login', async () => {
    it('it should login a user', async () => {
      expect(await page.$eval('#formTitle', e => e.innerText)).to.eql("Log in to your account");
  ...
  describe('/login errors', async () => {
    it('it should display form validation on all fields', async () => {
      ...
      await page.waitFor(1000);
      expect((await page.$eval('#pseudo', el => el.className)).includes('is-invalid')).eql(true);
      expect((await page.$eval('#password', el => el.className)).includes('is-invalid')).eql(true);
      expect(await page.$eval('#submitBtn', btn => btn.disabled)).eql(true);
      expect(page.url()).eql('http://localhost:4200/login');
    });
