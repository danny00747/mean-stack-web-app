...
describe('E2E TESTS FOR SINGUP PAGE', async () => {
  ...
  describe('/POST Register', () => {
    it('it should singup a user', async () => {
      expect(await page.$eval('#formTitle', e => e.innerText)).to.eql("Create an Account");
  ...
  describe('/register errors', () => {
    it("it should display an error if the username's length is below 4", async () => {
      ...
      await page.waitFor(1000);
      expect((await page.$eval('#usernameError1', el => el.innerHTML)).includes('minimum')).eql(true);
      expect(await page.$eval('#submitBtn', btn => btn.disabled)).eql(true);
    });
    it("it should display an error if the email is invalid", async () => {
      ...
      await page.waitFor(1000);
      expect((await page.$eval('#emailInvalid', el => el.innerHTML)).includes('is not a valid email')).eql(true);
      expect(await page.$eval('#submitBtn', btn => btn.disabled)).eql(true);
    });
    it("it should display an error if the passwords don't match", async () => {
      ...
      await page.waitFor(1000);
      expect(await page.$eval('#passwordMatchError', el => el.innerHTML)).eql('Passwords do not match');
      expect(await page.$eval('#submitBtn', btn => btn.disabled)).eql(true);
    });
    it("it should display an error on any form field in an invalid state", async () => {
      ...
      expect(await page.$eval('#usernameError2', el => el.innerHTML)).eql('A username is required');
      expect(await page.$eval('#emailRequired', el => el.innerHTML)).eql('An Email is required');
      expect(await page.$eval('#passwordRequired', el => el.innerHTML)).eql('A password is required');
      expect(await page.$eval('#confirmPasswordError', el => el.innerHTML)).eql('Please confirm your password');
      expect(await page.$eval('#submitBtn', btn => btn.disabled)).eql(true);
    });
    it('it should display an error if the username or email alreay exists', async () => {
      ...
      await submit.click();
      await page.waitFor(1000);
      expect((await page.$eval('#flashMessages', e => e.innerHTML)).includes('alert')).eql(true);
