...
function buildMakeUser(isValidEmail, hashPassword, requiredParameter) {
  return ({
    username = requiredParameter('username'),
    email = requiredParameter('email'),
    password = requiredParameter('password'),
    level = 'A1',
    score = 0,
    role = 'student',
    reviews = []
    ...
    if (role !== roleEnum.ADMIN && role !== roleEnum.TEACHER && role !== roleEnum.STUDENT) {
