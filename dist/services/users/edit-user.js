...
exports.default = makeEditUserService;

var _domain = require("../../domain");

function makeEditUserService({
  userRepository
}) {
  return async ({
    id,
    ...
    const user = (0, _domain.makeUser)({ ...changes
    });
    ...
    if (existing.length !== 0 && id !== existing[0]._id.toString()) return {
      message: "A user with the same username or email already exists !"
    };
    if (findUser.reviews.length !== 0) findUser.reviews.forEach(x => x["author"] = user.getEmail());
